...
(defun
 core-find-window-by-buffer-name (buffer-name-pattern &optional exact-match)
...
Example:
  (core-find-window-by-buffer-name \"*Flymake diagnostics\")
  (core-find-window-by-buffer-name \"*Ilist*\" t)"
 (require 'cl-lib)
 (cl-find-if
  (lambda
   (window)
...
(defun
 core-close-window-by-buffer-name (buffer-name-pattern &optional exact-match)
...
Example:
 ...
 (let ((window (core-find-window-by-buffer-name buffer-name-pattern exact-match)))
   (when window (quit-window nil window) t)))

(defun
 core-resize-window-to-ratio (window width-ratio)
