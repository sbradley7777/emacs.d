...
Uses `process-file' for TRAMP compatibility - works for both local and remote files."
 (when
  ...
  (with-temp-buffer
   (logging-debug "Python version check: %s" (abbreviate-file-name python-executable))
     ...
     (let ((output (string-trim (buffer-string))))
       ...
       (if
        ...
        (let ((version (match-string 1)))
          ...
          version)
        (logging-debug "Failed to parse version from: %s" output) nil))))))
 ...
 (let ((current-dir (or start-dir default-directory)))
   ...
   (let ((venv-path (python-utils-find-venv-path current-dir)))
     (if
      venv-path
      (progn
       (logging-success "Found Python venv at: %s" (abbreviate-file-name venv-path)) venv-path)
  ...
  (let ((venv-python (expand-file-name python-pyvenv-python-executable-path pyvenv-virtual-env)))
    (if
     ...
     (let ((local-python-path (file-local-name venv-python)))
       (logging-debug "Setting python-shell-interpreter to '%s' for remote venv" local-python-path)
