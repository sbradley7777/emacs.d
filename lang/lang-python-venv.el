...
(defvar config-python-project-name nil
...
(defun config-pyvenv-post-deactivate ()
...
(defun config-pyvenv-post-activate ()
...
(when (featurep 'pyvenv)
  ...
  (setq pyvenv-mode-line-indicator '(pyvenv-virtual-env-name ("[venv: " config-python-project-name "] ")))
...
(defun pyvenv-auto-activate ()
  ...
  (let* ((current-dir (expand-file-name default-directory))
         (project-root
          (or (locate-dominating-file current-dir ".git")
...
(add-hook
 ...
 (lambda ()
   ...
   (when (and (boundp 'pyvenv-virtual-env) pyvenv-virtual-env config-python-project-name)
     (force-mode-line-update))))

;; Make this module available for loading with (require 'lang-python-venv)
(provide 'lang-python-venv)
(message "lang-python-venv.el loaded (%.2fs)" (float-time (time-subtract (current-time) config-load-start-time)))
