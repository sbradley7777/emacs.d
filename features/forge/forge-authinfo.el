...
Does not run when editing remote files via TRAMP."
 ...
 (if
  ...
  (logging-warning "forge-authinfo-generate-entries disabled for remote files")
      ...
      (dolist
       ...
       (let* ((config (git-forge-config-parse-host-config host))
         ...
         (cond
          ...
          ((not api-host)
           (logging-warning "Host '%s' missing apihost in ~/.gitconfig, skipping" host)
           (setq skipped-count (1+ skipped-count)))
          ((member api-host existing-machines)
           (logging-info "Host '%s' already has credentials in ~/.authinfo" api-host)
           (setq skipped-count (1+ skipped-count)))
          (t
           (let* ((username
             ...
             (if
              ...
              (progn
               ...
               (logging-success "Prepared entry for %s" api-host))
              (logging-warning "Skipping %s (empty username or token)" api-host)
              (setq skipped-count (1+ skipped-count))))))))
      (if
       (null new-entries) (logging-info "No new entries to add to ~/.authinfo")
       (condition-case err
           (progn
...
(logging-config "Forge authinfo generator loaded")
