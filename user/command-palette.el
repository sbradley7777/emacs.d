...
(defconst
 ...
 '(("User Git Commit Format" . user-git-commit-format)
   ...
   ("Diagnostics Show External Dependencies" . diagnostics-show-external-dependencies)
   ...
   ("Diagnostics Show Package Repositories" . diagnostics-show-package-repositories)
   ("Diagnostics Show Package System" . diagnostics-show-package-system)
  ...
  (condition-case err
      (progn
       ...
       (when
        ...
        (unless
         silent
         (core-message-success "Loaded %d favorite command(s)" (length command-palette-favorites)))
        t))
    (error
