...
emacs --batch --load ~/github/emacs.d/core/package-manager.el --eval "(message \"Package manager test\")"
```
  ...
  test:
    ...
    steps:
      ...
      - name: Test Configuration
        run: ~/github/emacs.d/scripts/test-config.sh
```
...
```bash
#!/bin/bash
# Advanced CI script with multiple Emacs versions

EMACS_VERSIONS=("26.1" "27.2" "28.1" "29.1")

for version in "${EMACS_VERSIONS[@]}"; do
    echo "Testing with Emacs $version..."
    if emacs-$version --batch --load ~/github/emacs.d/init.el --eval "(message \"Test complete\")"; then
        echo "✅ Emacs $version: PASS"
    else
        echo "❌ Emacs $version: FAIL"
    fi
done
```
