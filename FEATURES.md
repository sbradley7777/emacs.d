...
| Key | Function | Description |
|-----|----------|-------------|
| `C-c` `C-p` | `run-python` | Start Python REPL in dedicated buffer |
| `C-c` `C-c` | `python-shell-send-buffer` | Execute entire file in REPL |
| `C-c` `C-r` | `python-shell-send-region` | Execute selected region in REPL |
| `C-M-a` | `python-nav-backward-defun` | Jump to previous function/class |
