...
(defun smart-tab ()
  "Smart TAB: complete if possible, otherwise indent."
  (interactive)
  (if (minibufferp)
      (minibuffer-complete)
...
(defun corfu-debug-info ()
  ...
  (when (bound-and-true-p eglot--managed-mode)
    (message "Eglot Server: %s" (eglot-current-server)))
  (message "==============================="))

;; Make this module available for loading with (require 'completion-corfu)
(provide 'completion-corfu)
(message "completion-corfu.el loaded (%.2fs)" (float-time (time-subtract (current-time) config-load-start-time)))
